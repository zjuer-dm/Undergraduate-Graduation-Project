...
def gen_seq_masks(seq_lens, max_len=None):
    if max_len is None:
        max_len = max(seq_lens) # 值比如为39
    batch_size = len(seq_lens) # len(tensor) 实际上返回的是 tensor.shape[0]，对于一维来说没区别，值比如为159
    device = seq_lens.device

    # 首先生成一个[1, max_len]的tensor值从0到max_len-1；然后将这个张量沿着第0维复制batch_size次，第1维复制1次（即第1维不变）得到形状为 [batch_size, max_len] 的张量
