...
def create_dataloaders(
    ...
    for k, task_name in enumerate(tasks):
        if task_name == 'mlm':
            task_dataset = MlmDataset(nav_db, tok) # MlmDataset对于每个episode都包含了完整路径下的观测，然后用于还原instruction
            ...
            task_dataset = GspDataset(nav_db, tok, end_vp_pos_ratio=0.15)
            task_collate_fn = gsp_collate
        else:
...
def main(opts):
    default_gpu, n_gpu, device = set_cuda(opts) # 单卡时，default_gpu==True；多卡时，只有第一张卡为True（local_rank==0）
    print(default_gpu, n_gpu, device)
    
    # 区分是否为default_gpu的作用是，有些事情只需要在一个进程上做就行，例如保存模型，记录log等，那么就设置在default_gpu进程上做
    if default_gpu:
        LOGGER.info(
    ...
    seed = opts.seed
    if opts.local_rank != -1: # 如果是多卡，那么每个gpu上的随机性都应该不同
        seed += opts.rank
    ...
    if default_gpu:
        ...
        # 在分布式训练中，只有 rank 0 的节点需要执行某些操作，如打印日志或保存模型，而其他节点则不需要执行这些操作。可以使用 NoOp 类来作为默认行为，确保其他节点不会执行这些操作。
        pbar = NoOp()
        model_saver = NoOp()
    ...
    # tokenizer = AutoTokenizer.from_pretrained(model_config.lang_bert_name, cache_dir='/home/ysh/.cache/huggingface/transformers/')
    # tokenizer = AutoTokenizer.from_pretrained("/home/ysh/habitat-lab-0.3.0/NAMI_ysh/Grounded-Segment-Anything/bert-base-uncased")
    ...
    if opts.checkpoint:
        ...
        if opts.init_pretrained == 'roberta':
            ...
            for param_name, param in tmp.items():
                ...
                if 'bert.encoder.layer' in param_name:
                    ...
                    if 'visual_attention' in param_name:
                        twin_param_name = param_name.replace('visual_attention', 'text_attention')
            ...
            if ln_weight_key in checkpoint:
                checkpoint['graph_attentioned_txt_embeds_transform.norm.weight'] = checkpoint[ln_weight_key].clone()
                print("Copy last visn_output.LayerNorm.weight to graph_attentioned_txt_embeds_transform.norm.weight")
            else:
                print(f"No {ln_weight_key} found")
            if ln_bias_key in checkpoint:
                checkpoint['graph_attentioned_txt_embeds_transform.norm.bias'] = checkpoint[ln_bias_key].clone()
                print("Copy last visn_output.LayerNorm.bias to graph_attentioned_txt_embeds_transform.norm.bias")
            else:
    ...
    for step, (name, batch) in enumerate(meta_loader):
        ...
        task = name.split('_')[0] # split前后一样的，本来就是形如mlm，sap这种字符串
        # print(name, task)
        ...
        if name == "mlm":
            ...
            gsp_loss.append(loss.item())

        # optimizer update and logging
        if (step + 1) % opts.gradient_accumulation_steps == 0:
            global_step += 1 # opts.gradient_accumulation_steps才算一次global_step
            ...
            if global_step % opts.log_steps == 0:
                ...
                for t in train_dataloaders.keys():
                    ...
                    TB_LOGGER.add_scalar(f'perf/{t}_loss_per_s', l_per_sec,
                                         global_step)
                LOGGER.info('===============================================')
                LOGGER.info(f"MLM mean loss during this log interval: {np.mean(mlm_loss)}")
                LOGGER.info(f"SAP mean loss during this log interval: {np.mean(sap_loss)}")
                mlm_loss = []
                sap_loss = []
                gsp_loss = []
