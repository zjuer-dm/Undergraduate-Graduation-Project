...
def load_parser():
    ...
    # local_rank是在这里被初始化为-1（每个gpu进程都是，后续具体赋值在下方的args = parser.parse_args()中）
    parser.add_argument(
        "--local_rank",
        type=int,
        default=-1,
...
def parse_with_config(parser):
    args = parser.parse_args() # parser.parse_args()解析命令行参数，每个进程不同的local_rank被自动包含在了命令行参数中。在这里每个gpu进程的local_rank被赋予了不同的具体值，从0开始到gpu数-1
