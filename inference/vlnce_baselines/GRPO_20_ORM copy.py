...
@baseline_registry.register_trainer(name="GRPO-20-ORM")
class RLTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        self.grpo_epsilon = config.GRPO_ORM.get("grpo_epsilon", 0.2)  # Clipping for PPO objective
        self.grpo_beta = config.GRPO_ORM.get("grpo_beta", 0.01)    # Coefficient for KL divergence loss
        self.max_grad_norm = config.GRPO_ORM.get("max_grad_norm", 1.0)
        ...
        self.grpo_update_epochs = config.GRPO_ORM.get("update_epochs", 4)
        ...
        for epoch in range(self.grpo_update_epochs):
            ...
            for s_idx in range(self.config.GRPO_ORM.sample_num):
                ...
                if total_actions_in_this_sample > 0:
                    ...
                    scaled_loss_for_this_sample.backward() # 对缩放后的loss进行反向传播
    ...
    def sample_once(self, initial_obs):
        ...
        for stepk in range(self.max_len): # 15
            ...
            # ------------------- start store data ------------------- 
            data_this_stepk = {}
            data_this_stepk["input"] = nav_inputs_copy # dict
            data_this_stepk["action"] = a_t.cpu() # 长为self.envs.num_envs的一维tensor，不包含梯度，对于在gpu上的变量，.cpu()直接创建一个新的变量，因此a_t变了也没事
            data_this_stepk["probs"] = nav_probs.cpu() # self.envs.num_envs * action_nums的二维tensor，不包含梯度
            ...
            for i in range(self.envs.num_envs):
                ...
                # distance_to_goal_reward = 1 - (metric['distance_to_goal']) / 3
                if metric['distance_to_goal'] > 3:
