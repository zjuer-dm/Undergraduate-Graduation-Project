...
@baseline_registry.register_trainer(name="GRPO-20-ORM-old-ok-T-trainable")
class RLTrainer(BaseVLNCETrainer):
    ...
    def setup_training_parts(self):
        ...
        if hasattr(self, 'trainable_individual_params') and self.trainable_individual_params:
            print("Setting up specific individual parameters for training...")
            for param_to_train in self.trainable_individual_params:
                param_to_train.requires_grad = True
                # 注意：我们不需要也无法对一个参数调用 .train()
                print(f"  Individual parameter UNFROZEN.")
    ...
    def _initialize_policy(
        ...
        try:
            ...
            if self.train_all:
                ...
                self.trainable_individual_params = []
            else:
                ...
                self.trainable_individual_params = [vln_bert_module.log_temp]
            # 验证模块是否确实是 nn.Module
        ...
        base_lr = self.config.GRPO_ORM.lr
        log_temp_lr = base_lr * 50
        
        # 2. 区分 log_temp 参数和其他所有可训练参数
        #    首先获取 log_temp 参数对象的直接引用
        try:
            if self.config.GPU_NUMBERS > 1:
                log_temp_param = self.policy.net.module.vln_bert.log_temp
            else:
                log_temp_param = self.policy.net.vln_bert.log_temp
            self.log_temp_param = log_temp_param
        except AttributeError:
            print("Error: Could not find log_temp parameter. Aborting optimizer setup.")
            self.optimizer = None
            return # 或者进行其他错误处理
        ...
        for param in self.policy.parameters():
            ...
            # 通过对象ID来判断是否是同一个参数
            # 'is' 运算符检查对象身份，比 '==' 更精确
            if param is log_temp_param:
                log_temp_params.append(param)
            else:
                other_trainable_params.append(param)

        # 3. 创建参数组 (Parameter Groups)
        optimizer_grouped_parameters = [
            ...
            {
                'params': log_temp_params,
                'lr': log_temp_lr
            }
        ]
        
        # 过滤掉没有参数的组，以防 log_temp 未被设为可训练
        optimizer_grouped_parameters = [g for g in optimizer_grouped_parameters if g['params']]

        # 4. 使用参数组来初始化优化器
        self.optimizer = torch.optim.AdamW(
            optimizer_grouped_parameters, 
            lr=base_lr # 这里的 lr 是一个默认值，会被组内指定的 lr 覆盖
        )
        
        print("Optimizer configured with parameter groups:")
        print(f"  - Group 0 (Other Params): {len(other_trainable_params)} params, lr = {base_lr}")
        if log_temp_params:
            print(f"  - Group 1 (log_temp): {len(log_temp_params)} params, lr = {log_temp_lr}")
        ...
        if self.need_ref_policy:
            ...
            for param in self.ref_policy.parameters():
                param.requires_grad = False
            self.ref_policy.load_state_dict(self.policy.state_dict(), strict=False)
        else:
    ...
    def update(self):
        ...
        for epoch in range(self.grpo_update_epochs):
            ...
            for s_idx in range(self.config.GRPO_ORM.sample_num):
                ...
                with autocast(enabled=self.enable_amp):
                    for step_data in current_sample_trajectory_steps_data:
                        ...
                        with autocast(enabled=False):
                            ...
                            # policy_loss_this_step = -torch.min(surr1, surr2).sum() 
                            ...
                            if self.need_ref_policy:
                                ...
                                # kl_div_this_step = (ratio_ref_over_current - log_ratio_ref_over_current - 1).sum()
                ...
                if num_valid_steps_for_this_sample > 0:
                    ...
                    # if self.policy.net.vln_bert.log_temp.grad is not None:
                    #     print(f"Current log_temp value 1: {self.policy.net.vln_bert.log_temp.data}")
                    #     print(f"Gradient of log_temp 1: {self.policy.net.vln_bert.log_temp.grad}")
            ...
            if num_samples_processed_this_epoch == self.config.GRPO_ORM.sample_num:
                ...
                trainable_params = [p for p in self.policy.parameters() if p.requires_grad and p is not self.log_temp_param]
                # trainable_params = [p for p in self.policy.parameters() if p.requires_grad]
                if trainable_params:
                    # print("check T 1 ", self.policy.net.vln_bert.log_temp.grad)
                    grad_norm = torch.nn.utils.clip_grad_norm_(trainable_params, self.max_grad_norm)
                    self.logs['grad_norm'].append(grad_norm.item())
                    # print("check T 2 ", self.policy.net.vln_bert.log_temp.grad)
                    # print("grad_norm: ", grad_norm)
                
                # if self.policy.net.vln_bert.log_temp.grad is not None:
                #     print(f"Current log_temp value 2: {self.policy.net.vln_bert.log_temp.data}")
                #     print(f"Gradient of log_temp 2: {self.policy.net.vln_bert.log_temp.grad}")
        ...
        current_T = current_policy_outputs['T']
        self.logs['current_T'].append(current_T)
