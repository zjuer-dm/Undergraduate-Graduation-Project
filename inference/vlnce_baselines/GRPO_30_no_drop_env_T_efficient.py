...
@baseline_registry.register_trainer(name="GRPO-30-no-drop-env-T-efficient")
class RLTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        self.T_start = config.GRPO_ORM.T_start
        self.T_end = config.GRPO_ORM.T_end
        self.T_decay_steps = config.GRPO_ORM.iters
        self.current_temperature = self.T_start
        self.global_step = 0 # 用于温度衰减的全局步数计数器
    ...
    def _update_temperature(self):
        """ 使用指数衰减策略更新当前温度 """
        if self.global_step < self.T_decay_steps:
            progress = self.global_step / self.T_decay_steps
            self.current_temperature = self.T_start * ((self.T_end / self.T_start) ** progress)
        else:
            self.current_temperature = self.T_end
        ...
        for idx in range(start_iter, total_iter, log_every):
            ...
            if self.local_rank < 1:
                ...
                for k, v in logs.items():
                    ...
                    writer.add_scalar(f'grpo/{k}', logs[k], cur_iter)
                current_lr = self.optimizer.param_groups[0]['lr']
                writer.add_scalar('train/lr', current_lr, cur_iter)
                writer.add_scalar('train/temperature', self.current_temperature, cur_iter)
                logger.info(loss_str)
                logger.info(f"lr: {current_lr}, temperature: {self.current_temperature:.4f}")
        ...
        for epoch in range(self.grpo_update_epochs):
            ...
            for s_idx in range(self.config.GRPO_ORM.sample_num):
                ...
                with autocast(enabled=self.enable_amp):
                    for step_data in current_sample_trajectory_steps_data:
                        ...
                        with autocast(enabled=False):
                            ...
                            tempered_current_logits = current_logits / self.current_temperature
                            current_log_probs = F.log_softmax(tempered_current_logits, dim=1)
                            ...
                            if self.need_ref_policy:
                                ...
                                tempered_ref_logits = ref_logits / self.current_temperature
                                ref_log_probs = F.log_softmax(tempered_ref_logits, dim=1)
                ...
                if num_valid_steps_for_this_sample > 0:
                    ...
                    if (s_idx < self.config.GRPO_ORM.sample_num - 1) and (self.world_size > 1):
    ...
    def sample_once(self, initial_obs):
        ...
        for stepk in range(self.max_len): # 15
            ...
            tempered_logits = nav_logits / self.current_temperature
            nav_probs = F.softmax(tempered_logits, 1)
            ...
            if self.config.MODEL.task_type == 'r2r':
                ...
                for i in range(self.envs.num_envs):
                    ...
                    distance_to_goal_reward = 1 - 1 * (metric['distance_to_goal']) / 6
                    data_this_sample["reward"][not_done_index[i]] = metric['ndtw'] + metric['sdtw'] + distance_to_goal_reward
                    ...
                    # print(f"check: ndtw:{metric['ndtw']}, sdtw:{metric['sdtw']}, NE:{metric['distance_to_goal']}")
            # pause env
            if sum(dones) > 0:
