...
class RLTrainer(BaseVLNCETrainer):
    ...
    def update(self):
        ...
        for epoch in range(self.grpo_update_epochs):
            ...
            all_current_log_probs_taken_action = []
            with autocast(enabled=self.enable_amp):
                # <--- 关键修改：同时遍历 nav_inputs 和对应的 taken_actions --->
                for nav_inputs, taken_actions in zip(all_nav_inputs, all_taken_actions):
                    current_policy_outputs = self.policy.net(**nav_inputs)
                    current_logits = current_policy_outputs['global_logits']
                    current_log_probs = F.log_softmax(current_logits, dim=1)
                    
                    # <--- 关键修改：在循环内部就使用gather提取出被采样动作的log_prob --->
                    # taken_actions 已经是 on device, 无需再 .to(device)
                    log_prob_for_taken_action = current_log_probs.gather(1, taken_actions.unsqueeze(1)).squeeze(1)
                    all_current_log_probs_taken_action.append(log_prob_for_taken_action)

            # Concatenate lists of tensors into single large tensors
            # <--- 关键修改：直接拼接已经提取好的、形状一致的log_probs --->
            current_log_probs_tensor = torch.cat(all_current_log_probs_taken_action)
            ...
            unique_advantages = torch.zeros(num_trajectories, device=self.device)
            unique_advantages.scatter_add_(0, traj_ids_tensor, advantages_tensor)

            # Avoid division by zero for trajectories of length 0 (should not happen)
            traj_lengths = torch.clamp(traj_lengths, min=1)

            # Manually compute the mean
            unique_advantages = unique_advantages / traj_lengths
