...
@baseline_registry.register_trainer(name="GRPO-30-no-drop-env-efficient")
...
```
My_Policy_ViewSelection_ETP_30.py文件
```py
from copy import deepcopy
import numpy as np
import time
...
from vlnce_baselines.models.etp.my_vlnbert_init_30 import get_vlnbert_models
from vlnce_baselines.common.aux_losses import AuxLosses
...
class MyPolicyViewSelectionETP30(ILPolicy):
    def __init__(
        self,
        observation_space: Space,
        action_space: Space,
...
class ETP(Net):
    def __init__(
        ...
        self.critic = Critic(drop_ratio=dropout_rate)
    ...
    def forward(self, mode=None, 
        ...
        if mode == 'language':
            ...
            # state_features = outs['gmap_embeds'][:, 0, :]
            # value = self.critic(state_features)
            # outs['value'] = value
            state_features = outs['gmap_embeds'][:, 0, :]
            value = self.critic(state_features) # critic 的输出现在已经是 (batch_size)
            outs['value'] = value.unsqueeze(-1) # 为了保持输出接口一致，把它变回 (batch_size, 1)
            return outs
