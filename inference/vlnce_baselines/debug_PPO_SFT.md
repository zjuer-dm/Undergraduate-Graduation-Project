...
flag1="--exp_name release_r2r_30
      --run-type train
      --exp-config run_r2r_30_PPO/iter_train.yaml
      SIMULATOR_GPU_IDS [0]
      TORCH_GPU_IDS [0]
      GPU_NUMBERS 1
      NUM_ENVIRONMENTS 2
...
flag2=" --exp_name release_r2r_30
      ...
      NUM_ENVIRONMENTS 8
      TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.ALLOW_SLIDING True
      EVAL.CKPT_PATH_DIR data/logs/checkpoints/release_r2r_30_grpo_all/release_r2r_30_grpo/ckpt.iter270.pth
      IL.back_algo control
      MODEL.pretrained_path pretrained/r2r_rxr_ce_30/mlm.sap_habitat_depth/store2/model_step_367500.pt
      "
...
flag_sft_for_ppo="--exp_name release_r2r_30_SFT_for_PPO
      --run-type train
      --exp-config run_r2r_30_PPO/iter_train.yaml
      SIMULATOR_GPU_IDS [0]
      TORCH_GPU_IDS [0]
...
case $mode in 
      sft)
      echo "###### SFT for PPO mode ######"
      python -m torch.distributed.launch --nproc_per_node=1 --master_port $2 run.py $flag_sft_for_ppo
      ;;
      train)
      echo "###### train mode ######"
      ...
      # python -m torch.distributed.launch --nproc_per_node=1 --master_port $2 vlnce_baselines/trainer.py
      ;;
      eval)
      echo "###### eval mode ######"
...
# 命令行运行：
# CUDA_VISIBLE_DEVICES=0 bash run_r2r_30_PPO/main.bash train 2333
# CUDA_VISIBLE_DEVICES=0 bash run_r2r_30_PPO/main.bash eval 2333
# CUDA_VISIBLE_DEVICES=0 bash run_r2r_30_PPO/main.bash infer 2333
# CUDA_VISIBLE_DEVICES=0 bash run_r2r_30_PPO/main.bash sft 2333
```
...
@baseline_registry.register_trainer(name="MY-SS-ETP-30")
class RLTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        super().__init__(config)
        self.max_len = int(config.IL.max_traj_len) #  * 0.97 transfered gt path got 0.96 spl
