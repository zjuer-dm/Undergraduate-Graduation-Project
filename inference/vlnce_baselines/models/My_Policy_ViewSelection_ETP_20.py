...
from vlnce_baselines.models.etp.my_vlnbert_init_20 import get_vlnbert_models
...
class MyPolicyViewSelectionETP20(ILPolicy):
    def __init__(
        self,
    ...
    @classmethod
    def from_config(
        cls, config: Config, observation_space: Space, action_space: Space, dropout_rate=0.1, need_gsp=True
    ):
        config.defrost()
...
class ETP(Net):
    def __init__(
        self, observation_space: Space, model_config: Config, num_actions, dropout_rate, need_gsp
    ):
        super().__init__()
        ...
        self.vln_bert = get_vlnbert_models(config=model_config, dropout_rate=dropout_rate, need_gsp=need_gsp)
