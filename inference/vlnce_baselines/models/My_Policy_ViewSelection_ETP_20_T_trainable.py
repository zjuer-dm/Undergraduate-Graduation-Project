...
from vlnce_baselines.models.etp.my_vlnbert_init_20_T import get_vlnbert_models
...
class MyPolicyViewSelectionETP20T_trainable(ILPolicy):
    def __init__(
        self,
    ...
    @classmethod
    def from_config(
        cls, config: Config, observation_space: Space, action_space: Space, dropout_rate=0.1, need_gsp=True, need_T=False, initial_T=5.0
    ):
        config.defrost()
...
class ETP(Net):
    def __init__(
        self, observation_space: Space, model_config: Config, num_actions, dropout_rate, need_gsp, need_T, initial_T
    ):
        super().__init__()
        ...
        self.vln_bert = get_vlnbert_models(config=model_config, dropout_rate=dropout_rate, need_gsp=need_gsp, need_T=need_T, initial_T=initial_T)
    ...
    def forward(self, mode=None, 
                txt_ids=None, subtask_type_ids=None, txt_masks=None, txt_embeds=None, 
                ...
                gmap_masks=None, gmap_visited_masks=None, gmap_pair_dists=None, subtask_lens=None):

        if mode == 'language':
            encoded_sentence = self.vln_bert.forward_txt(
                txt_ids, subtask_type_ids, txt_masks,
            )
