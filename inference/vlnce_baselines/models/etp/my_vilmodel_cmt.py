...
class CLSTransformBlock(nn.Module):
...
class GlocalTextPathNavCMT(BertPreTrainedModel): # CMT可能指的是cross model transformer；Text、Path、Nav分别指代对应不同任务的输出，即该class能同时输出多种任务
    ...
    def forward_navigation(
        ...
        cls_token = txt_embeds[:, 0]
        c_prime = self.cls_transform(cls_token)
        B, L_N, H = gmap_embeds.shape
        c_expand = c_prime.unsqueeze(1).expand(-1, L_N, -1)
        fusion_input = torch.cat([gmap_embeds, c_expand], dim=-1)
        ...
        global_stage_logits = self.global_stage_head(c_prime)
