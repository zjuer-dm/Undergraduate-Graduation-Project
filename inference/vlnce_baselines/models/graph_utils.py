...
class GraphMap(object):
    ...
    def identify_node(self, cur_pos, cur_ori, cand_ang, cand_dis, last_stepk_is_subtask_stop):
        # assume no repeated node
        # since action is restricted to ghosts
        if not last_stepk_is_subtask_stop:
            cur_vp = str(len(self.node_pos)) # 为新的点安排一个编号
            ...
            cur_vp = str(len(self.node_pos) - 1) # 这个编号指向上一个已经建立的节点
            cand_vp = [f'{cur_vp}_{str(i)}' for i in range(len(cand_ang))]
            cand_pos = [p for p in estimate_cand_pos(cur_pos, cur_ori, cand_ang, cand_dis)]
        return cur_vp, cand_vp, cand_pos
    def identify_node_new(self, cur_pos, cur_ori, cand_ang, cand_dis):
        # assume no repeated node
        # since action is restricted to ghosts
        cur_vp = str(len(self.node_pos)) # 为新的点安排一个编号
        cand_vp = [f'{cur_vp}_{str(i)}' for i in range(len(cand_ang))]
    ...
    def update_graph(self, prev_vp, step_id,
                           ...
                           cand_real_pos):
        # print("graph update start")
        # print("current_vp prev_vp", cur_vp, prev_vp)
        # 1. connect prev_vp
        ...
        # print("self.graph_nx ", self.graph_nx.nodes(), self.graph_nx.edges())
        ...
        for vp in gmap_vp_ids:
            ...
            try:
                ...
                if cur_vp == vp:
                    ...
                    shortest_dist = rel_dist # 使用直线距离作为近似
                    shortest_step = 1
