...
from vlnce_baselines.common.utils import extract_instruction_tokens_new
...
@baseline_registry.register_trainer(name="MY-SS-ETP")
class RLTrainer(BaseVLNCETrainer):
    ...
    def train(self):
        ...
        for idx in range(start_iter, total_iter, log_every):
            ...
            if self.config.IL.train_gsp:
                gsp_loss_weight = self.config.IL.gsp_loss_weight_low + (self.config.IL.gsp_loss_weight_high-self.config.IL.gsp_loss_weight_low)*(1-idx/total_iter)
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None, gsp_loss_weight=None):
        ...
        observations, global_subtask_num = extract_instruction_tokens_new(observations, self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID, # INSTRUCTION_SENSOR_UUID = instruction
        ...
        for stepk in range(self.max_len): # 15
            ...
            # obs for next step
            observations = extract_instruction_tokens_new(observations,self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID, \
