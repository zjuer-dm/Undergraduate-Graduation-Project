...
@baseline_registry.register_trainer(name="MY-SS-ETP-20")
class RLTrainer(BaseVLNCETrainer):
    ...
    def _teacher_stage_new(self, global_subtask_stage):
        for i, subtask_stage in enumerate(global_subtask_stage):
            current_subtask_stage = self.envs.call_at(i, "current_subtask_stage_update", {"last_subtask_stage": subtask_stage, "dis_thre": 1.5})
            # print(f"last stage: {subtask_stage}, current stage: {current_subtask_stage}")
            global_subtask_stage[i] = current_subtask_stage
        return global_subtask_stage
    ...
    def train(self):
        ...
        for idx in range(start_iter, total_iter, log_every):
            ...
            if self.config.IL.warm_up > 0:
                if idx < self.config.IL.warm_up:
                    sample_ratio = 1.0
                else:
                    sample_ratio = self.config.IL.sample_ratio ** ((idx-self.config.IL.warm_up) // self.config.IL.decay_interval + 1)
            else:
            ...
            if self.config.IL.train_gsp:
                if idx >= self.config.IL.gsp_loss_weight_lowest_iter:
                    gsp_loss_weight = self.config.IL.gsp_loss_weight_low
                else:
                    gsp_loss_weight = self.config.IL.gsp_loss_weight_low + (self.config.IL.gsp_loss_weight_high-self.config.IL.gsp_loss_weight_low)*(1-idx/self.config.IL.gsp_loss_weight_lowest_iter)
                print("gsp loss weight: ", gsp_loss_weight)
            else:
                gsp_loss_weight = None

            # sample_ratio = self.config.IL.sample_ratio ** (idx // self.config.IL.decay_interval)
            logs = self._train_interval(interval, self.config.IL.ml_weight, sample_ratio, gsp_loss_weight)
    ...
    def _train_interval(self, interval, ml_weight, sample_ratio, gsp_loss_weight):
        self.policy.train()
        if self.world_size > 1:
        ...
        for idx in pbar:
            ...
            with autocast():
                self.rollout('train', ml_weight, sample_ratio, gsp_loss_weight)
            # 每次rollout后都进行更新训练
        ...
        if self.config.IL.train_gsp:
            print(f"total sap loss: {self.sap_loss}, total gsp loss: {self.gsp_loss}, gsp loss weight 1: {self.gsp_loss / gsp_loss_weight}")
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None, gsp_loss_weight=None):
        if mode == 'train':
            feedback = 'sample'
        ...
        observations, global_subtask_num = extract_instruction_tokens_new_20(observations, self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID, # INSTRUCTION_SENSOR_UUID = instruction
        ...
        if mode == 'eval':
            ...
            self.envs, batch, global_subtask_num, global_subtask_stage = self._pause_envs(self.envs, batch, env_to_pause, global_subtask_num, global_subtask_stage)
        ...
        # print("episode subtask num: ", len(global_subtask_num[0]))
        for stepk in range(self.max_len): # 15
            # print("all_txt_ids ", all_txt_ids)
            # print("prev_vp ", prev_vp[0])
            # print("num_envs: ", self.envs.num_envs)
            # print("len(global_subtask_num[0]) ", len(global_subtask_num[0]))
            # print("global_subtask_stage[0]", global_subtask_stage[0])
            # print("HERE\n", (self.envs.current_episodes())[0].episode_id, global_subtask_stage)
            ...
            nav_inputs.update({
                'mode': 'navigation',
                'txt_embeds': txt_embeds, # requires_grad为True
                'txt_masks': txt_masks, # requires_grad为False
            ...
            if mode == 'train' or self.config.VIDEO_OPTION:
                ...
                global_subtask_stage = self._teacher_stage_new(global_subtask_stage)
            if mode == 'train': # 此处计算loss，-100是动作空间为空的情况；计算loss就是根据teacher_action作为真值，而后续的动作执行则是依据概率进行采样
                # loss += F.cross_entropy(nav_logits, teacher_actions, reduction='sum', ignore_index=-100)
                if self.config.IL.train_gsp:
                    global_subtask_stage_tensor = torch.LongTensor(global_subtask_stage).cuda()
                    sap_loss = F.cross_entropy(nav_logits, teacher_actions, reduction='sum', ignore_index=-100)
                    gsp_loss = gsp_loss_weight * F.cross_entropy(stage_logits, global_subtask_stage_tensor, reduction='sum')
                    if bool(torch.isinf(gsp_loss)):
                        # inf是因为instruction长度超出80被截断，然后1064这个token也缺失了，因此导致subtask_num错误
                        # print("gsp inf!!! ", stage_logits, global_subtask_stage_tensor)
                        # print("episodes ", self.envs.current_episodes())
                        loss += sap_loss
                    else:
                        loss += sap_loss + gsp_loss
                        self.gsp_loss += gsp_loss.item()
                    # print(f"sap loss: {sap_loss}, gsp loss: {gsp_loss}")
                    self.sap_loss += sap_loss.item()
            ...
            if feedback == 'sample':
                ...
                if self.config.IL.force_stop:
                    a_t[a_t == 0] = teacher_actions[a_t == 0] # 不允许随意stop，stop的位置只能是teacher_actions认为可以stop的位置（1.5m以内）
            ...
            if mode == 'eval':
                ...
                for i in range(self.envs.num_envs):
                    ...
                    gt_length = distances[0] # R2R-CE的数据集，在RXR-CE的设置下，出现inf
                    # if not np.isfinite(gt_length):
                    #     print(f"发现非有限值: {gt_length}，需要特殊处理！")
