...
@baseline_registry.register_trainer(name="MY-SS-ETP-30-SMOOTH")
class RLTrainer(BaseVLNCETrainer):
    ...
    def _initialize_policy(
        ...
        num_training_steps = self.config.IL.iters # 总训练步数
        min_lr_ratio = self.config.IL.min_lr_ratio # 最小学习率为主学习率的10%
    ...
    def _teacher_action_new(self, batch_gmap_vp_ids, batch_no_vp_left, is_train):
        ...
        for i, (gmap_vp_ids, gmap, no_vp_left) in enumerate(zip(batch_gmap_vp_ids, self.gmaps, batch_no_vp_left)):
            ...
            if curr_dis_to_goal < 1.5: # 1.5m内认为应该停止
                teacher_actions.append(0)
            else:
                if no_vp_left: # 没有动作空间时给的动作值为-100
                    teacher_actions.append(-100)
                elif self.config.IL.expert_policy == 'spl': # 默认使用的是spl
                    # gmap.ghost_real_pos中每一个节点的pos可能有多个，因此使用random.choice随机选一个全局坐标
                    ghost_vp_pos = [(vp, random.choice(pos)) for vp, pos in gmap.ghost_real_pos.items()]
                    ghost_dis_to_goal = [
                        self.envs.call_at(i, "point_dist_to_goal", {"pos": p[1], "is_train": is_train}) # self.envs.call_at里面的编号也遵从下面rollout的规则，也就是停止了就将其移除
                    ...
                    # })
                    ghost_dists_to_subgoal = self.envs.call_at(i, "ghost_dist_to_ref", {
                        "ghost_vp_pos": ghost_vp_pos,
                        "ref_path": self.gt_data[str(cur_episodes[i].episode_id)]['locations'],
                    })
                    target_ghost_vp = ghost_vp_pos[np.argmin(ghost_dists_to_subgoal)][0]
                    teacher_actions.append(gmap_vp_ids.index(target_ghost_vp))
                else:
                    raise NotImplementedError
                # if self.local_rank < 1:
                #     print("ghost_dis_to_goal ", ghost_dis_to_goal)
                #     print("teacher_actions ", teacher_actions)
        return torch.tensor(teacher_actions).cuda()

    def _teacher_action_new_with_dists(self, batch_gmap_vp_ids, batch_no_vp_left, is_train):
        teacher_actions = []
        all_ghost_dists = [] # 新增：用于存储每个环境的距离列表
        cur_episodes = self.envs.current_episodes()

        for i, (gmap_vp_ids, gmap, no_vp_left) in enumerate(zip(batch_gmap_vp_ids, self.gmaps, batch_no_vp_left)):
            
            # 即使teacher决定stop，我们仍然需要计算所有ghost的距离用于标签平滑
            if no_vp_left:
                teacher_actions.append(-100)
                all_ghost_dists.append([]) # 没有候选点，距离列表为空
                continue

            ghost_vp_pos = [(vp, random.choice(pos)) for vp, pos in gmap.ghost_real_pos.items()]
            # print("ghost_vp_pos ", ghost_vp_pos)
            
            if self.config.IL.expert_policy == 'spl': # 默认使用的是spl
                ghost_dis_to_goal = [
                    self.envs.call_at(i, "point_dist_to_goal", {"pos": p[1], "is_train": is_train})
                    for p in ghost_vp_pos
                ]
            elif self.config.IL.expert_policy == 'ndtw':
                ghost_dis_to_goal = self.envs.call_at(i, "ghost_dist_to_ref", {
                    "ghost_vp_pos": ghost_vp_pos,
                    "ref_path": self.gt_data[str(cur_episodes[i].episode_id)]['locations'],
                })
            else:
            ...
            all_ghost_dists.append(ghost_dis_to_goal) # 存储当前环境的距离列表
            ...
            # print("ghost_dis_to_goal ", ghost_dis_to_goal)
            gi_list = []
            for g in ghost_vp_pos:
                gn = g[0]
                gi = gmap_vp_ids.index(gn)
                gi_list.append(gi)
        #     print("gi_list ", gi_list)
        # print("all_ghost_dists ", all_ghost_dists)

        return torch.tensor(teacher_actions).cuda(), all_ghost_dists
    ...
    def _nav_gmap_variable(self, cur_vp, cur_pos, cur_ori, task_type):
        ...
        for i, gmap in enumerate(self.gmaps):
            ...
            batch_num_nodes.append(len(node_vp_ids))
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        # observations[i]的keys：
        # ['rgb', 'depth', 'rgb_30', 'rgb_60', 'rgb_90', 'rgb_120', 'rgb_150', 'rgb_180', 'rgb_210', 'rgb_240', 'rgb_270', 'rgb_300', 'rgb_330', 'depth_30', 
        # 'depth_60', 'depth_90', 'depth_120', 'depth_150', 'depth_180', 'depth_210', 'depth_240', 'depth_270', 'depth_300', 'depth_330', 'instruction']
        observations = self.envs.reset()

        # cp = self.envs.current_episodes()[0].goals
        # print("instruction:\n", cp)


        # show_img = observations[0]["rgb"]
        # cv2.imshow("q", show_img)
        # cv2.waitKey(0)
        ...
        observations = extract_instruction_tokens_new_30(observations, self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID, # INSTRUCTION_SENSOR_UUID = instruction
        ...
        if mode == 'eval':
            ...
            # print("env_to_pause: ", env_to_pause)
            self.envs, batch = self._pause_envs(self.envs, batch, env_to_pause)
            if self.envs.num_envs == 0: return
        if mode == 'infer':
            env_to_pause = [i for i, ep in enumerate(self.envs.current_episodes()) 
                            if ep.episode_id in self.path_eps] # self.path_eps中记录了所有已经完成的episode_id，如果某个env重置后是已经完成的episode，说明这个env已经完全跑完了
            # 返回的batch中每样value的batch_num变成了只有未暂停的envs数，但在此处刚初始化，因此不会改变batch尺寸
            self.envs, batch = self._pause_envs(self.envs, batch, env_to_pause) 
            if self.envs.num_envs == 0: return
        ...
        # print("check masks: ", all_txt_masks)
        # print("check all_txt_task_encoding: ", all_txt_task_encoding)
        ...
        for stepk in range(self.max_len): # 15
            ...
            wp_outputs = self.policy.net(
                ...
                observations = batch,
                in_train = (mode == 'train' and self.config.IL.waypoint_aug), # eval或infer时，为False；train时，为True
            )
            ...
            # 首先，调用 _nav_gmap_variable 时需要接收新的返回项
            nav_inputs = self._nav_gmap_variable(cur_vp, cur_pos, cur_ori, task_type)
            # 从 nav_inputs 中 pop 出非模型输入的辅助信息
            no_vp_left = nav_inputs.pop('no_vp_left')
            if any(no_vp_left):
                print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                print("!!! no_vp_left!   !!!", no_vp_left)
                print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            batch_num_nodes = torch.tensor(nav_inputs.pop('num_nodes'), device=self.device) # 转换为Tensor
            nav_inputs.update({
                'mode': 'navigation',
            ...
            if torch.isnan(nav_logits).any():
                print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                print("!!! NaN or Inf detected in nav_logits!   !!!")
                print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                # 您可以在这里选择性地保存一些中间变量用于分析，然后主动退出
                print("nav_logits ", nav_logits)
                import sys
                sys.exit("Exiting due to NaN/Inf in model output.")

            nav_probs = F.softmax(nav_logits, 1)
            for i, gmap in enumerate(self.gmaps):
                gmap.node_stop_scores[cur_vp[i]] = nav_probs[i, 0].data.item()
            if self.config.VIDEO_OPTION or mode == 'train':
                teacher_actions, all_ghost_dists = self._teacher_action_new_with_dists(
                    nav_inputs['gmap_vp_ids'], no_vp_left, mode == 'train'
                )
            if mode == 'train':
                if not self.config.IL.USE_LABEL_SMOOTHING:
                    loss += F.cross_entropy(nav_logits, teacher_actions, reduction='sum', ignore_index=-100)
                else:
                    # --- 最终正确且高效的标签平滑损失计算逻辑 ---
                    # 0. 准备数据和Mask
                    alpha = self.config.IL.LS_ALPHA
                    T = self.config.IL.LS_TEMPERATURE
                    stop_prob = self.config.IL.LS_STOP_PROB
                    batch_size, max_num_actions = nav_logits.shape

                    num_cands = [len(d) for d in all_ghost_dists]
                    max_cands = max(num_cands) if num_cands else 0
                    padded_dists = torch.full((batch_size, max_cands), float('inf'), device=self.device)
                    dists_mask = torch.zeros((batch_size, max_cands), dtype=torch.bool, device=self.device)
                    for i, dists in enumerate(all_ghost_dists):
                        if dists: # 反面是没有动作空间的情况
                            padded_dists[i, :len(dists)] = torch.tensor(dists, device=self.device)
                            dists_mask[i, :len(dists)] = True
                    
                    valid_batch_mask = teacher_actions != -100
                    is_stop_batch = teacher_actions == 0

                    # 关键修正：计算 ghost 节点在 logits 中的动态起始索引
                    # 起始索引 = 1 (stop) + num_visited_nodes
                    ghost_start_indices = (1 + batch_num_nodes).unsqueeze(1)
                    # 构建用于 scatter 的动态索引矩阵 (batch_size, max_cands)
                    # 每一行都是 [start_idx, start_idx+1, start_idx+2, ...]
                    scatter_indices = ghost_start_indices + torch.arange(max_cands, device=self.device).unsqueeze(0)
                    assert scatter_indices.max().item() < nav_logits.shape[1], \
                        f"scatter index {scatter_indices.max().item()} out of range {nav_logits.shape[1]}"
                    # scatter_indices.clamp_max_(nav_logits.shape[1] - 1)

                    # 1. 处理非停止动作 (Vectorized & Correct Indexing)
                    # teacher_one_hot = F.one_hot(teacher_actions, num_classes=max_num_actions).bool()
                    dists_for_smooth = padded_dists.clone()
                    # 这里的 teacher_one_hot[:, 1:] 范围太大了，需要修正
                    # teacher_one_hot_for_cands = teacher_one_hot[:, scatter_indices] 这样不对
                    # 重新构造 teacher_one_hot_for_cands
                    # relative_teacher_idx = teacher_actions - ghost_start_indices.squeeze(1)
                    # teacher_one_hot_for_cands = F.one_hot(relative_teacher_idx.clamp(min=0), num_classes=max_cands).bool()
                    
                    # dists_for_smooth.masked_fill_(~dists_mask | teacher_one_hot_for_cands, float('inf'))
                    dists_for_smooth.masked_fill_(~dists_mask, float('inf'))
                    smooth_probs = F.softmax(-dists_for_smooth / T, dim=1) * alpha
                    smooth_probs[torch.isnan(smooth_probs)] = 0.0
                    
                    non_stop_targets = torch.zeros_like(nav_logits)

                    indices_for_scatter = teacher_actions.clone()
                    indices_for_scatter[teacher_actions <= 0] = 0  # 将 <= 0 的索引全部指向第0列

                    # 步骤 B: 创建一个源(src)张量。
                    # 只有在 teacher_actions > 0 的地方，值才是 1.0 - alpha，其他地方都是 0.0。
                    src_for_scatter = torch.zeros_like(teacher_actions, dtype=non_stop_targets.dtype)
                    src_for_scatter[teacher_actions > 0] = 1.0 - alpha

                    # 步骤 C: 在完整的原始张量上执行scatter。
                    # 这个操作会将 1.0 - alpha 写入到有效动作对应的正确位置。
                    # 对于无效或停止的动作，它会无害地将 0.0 写入到第0列。
                    non_stop_targets.scatter_(1, indices_for_scatter.unsqueeze(1), src_for_scatter.unsqueeze(1))

                    # nonstop_mask = (teacher_actions > 0) & valid_batch_mask
                    # if nonstop_mask.any():
                    #     ta = teacher_actions[nonstop_mask]
                    #     non_stop_targets[nonstop_mask].scatter_(1, ta.unsqueeze(1), 1.0 - alpha)
                    # else:
                    #     print("PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP")
                    # non_stop_targets.scatter_(1, teacher_actions.unsqueeze(1), 1.0 - alpha)
                    # 使用正确的 scatter_indices 将概率分布到正确的位置
                    if max_cands:
                        non_stop_targets.scatter_add_(1, scatter_indices, (smooth_probs * dists_mask.float()).to(non_stop_targets.dtype))
                    # non_stop_targets.scatter_add_(1, scatter_indices, smooth_probs * dists_mask.float())
                    
                    # 2. 处理停止动作 (Vectorized & Correct Indexing)
                    good_alts_mask = (padded_dists < 1.5) & dists_mask
                    has_alts = good_alts_mask.any(dim=1)
                    
                    dists_for_stop_smooth = padded_dists.clone()
                    dists_for_stop_smooth.masked_fill_(~good_alts_mask, float('inf'))
                    stop_smooth_probs = F.softmax(-dists_for_stop_smooth / T, dim=1) * (1.0 - stop_prob)
                    stop_smooth_probs[torch.isnan(stop_smooth_probs)] = 0.0

                    stop_targets = torch.zeros_like(nav_logits)
                    stop_targets[:, 0] = torch.where(has_alts, stop_prob, 1.0) # 通过torch.where实现了是否平滑的设置
                    # stop_targets.scatter_add_(1, scatter_indices, stop_smooth_probs * dists_mask.float())
                    if max_cands:
                        stop_targets.scatter_add_(1, scatter_indices, (stop_smooth_probs * dists_mask.float()).to(stop_targets.dtype))

                    # 3. 合并结果
                    final_targets = torch.where(is_stop_batch.unsqueeze(1), stop_targets, non_stop_targets)
                    
                    # 4. 计算损失
                    valid_logits = nav_logits[valid_batch_mask]
                    valid_targets = final_targets[valid_batch_mask]
                    
                    if valid_logits.numel() > 0:
                        log_probs = F.log_softmax(valid_logits, dim=1)
                        safe_log_probs = log_probs.clone()
                        safe_log_probs[log_probs == -float('inf')] = 0.0
                        ls_loss = -torch.sum(valid_targets * safe_log_probs)
                        # ls_loss = -torch.sum(valid_targets * log_probs)

                        if torch.isnan(ls_loss) or torch.isinf(ls_loss):
                            print("!!! ls_loss is NaN or inf! Problem in label smoothing logic. !!!")
                            # 保存相关张量用于调试
                            torch.save({'targets': valid_targets, 'log_probs': safe_log_probs}, 'nan_debug.pt')
            ...
            if feedback == 'sample':
                ...
                a_t = c.sample().detach()
                # torch.where(condition, x, y):如果 condition 为 True，返回 x；否则返回 y
                # 由torch.rand_like(a_t, dtype=torch.float)生成一个与 a_t 形状相同的随机张量，每个值在 [0, 1) 之间;
                # 并通过与sample_ratio比较决定哪些量要用teacher_action,哪些用概率采样结果
                # 因此最终的结果是,sample_ratio越来越低,使用teacher_action概率越来越低
            ...
            # make equiv action
            env_actions = []
            use_tryout = (self.config.IL.tryout and not self.config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.ALLOW_SLIDING) # 设置tryout并且不允许滑动才启用use_tryout
            for i, gmap in enumerate(self.gmaps):
                if cpu_a_t[i] == 0 or stepk == self.max_len - 1 or no_vp_left[i]: # 如果的当前动作是stop或步数达到上限或没有动作空间
                    ...
                    if self.config.IL.back_algo == 'control': # eval和infer进的是这里，也是通过节点路径走过去的
            ...
            if mode == 'eval':
                ...
                for i in range(self.envs.num_envs):
                    ...
                    # print("collisions: ", ep_id, info['collisions']['count'], len(pred_path))
                    ...
                    # print("QQQQQQQQQ    ", info.keys())
                    # show_final_pose = {
                    #         'position': info['position_infer']['position'][-1],
                    #         'heading': info['position_infer']['heading'][-1],
                    #     }        
                    # print("Episode " + str(ep_id) + "'s final_pose: ", show_final_pose)
                #     if len(info['position_infer']['position']) > 500:
                #         self.illegal_episodes_count = self.illegal_episodes_count + 1
                # print("CURRENT ILLEGAL: ", self.illegal_episodes_count)
            # record path
            if mode == 'infer':
                curr_eps = self.envs.current_episodes()
                for i in range(self.envs.num_envs):
        ...
        # 这一段在for循环外面,属于最后的后处理
        if mode == 'train':
            loss = ml_weight * loss / total_actions # total_actions表示此次rollout,所有envs下一共进行了几次上层决策;因此这个式子最后得到的是平均每次上层决策的loss大小
