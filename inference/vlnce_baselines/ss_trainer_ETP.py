...
class RLTrainer(BaseVLNCETrainer):
    ...
    @staticmethod
    def _pause_envs(envs, batch, envs_to_pause, global_token_list, global_text_stage):
        if len(envs_to_pause) > 0:
            ...
            # print("before ", global_token_list, global_text_stage)
            global_token_list = [global_token_list[i] for i in state_index]
            global_text_stage = [global_text_stage[i] for i in state_index]
            # print("after ", global_token_list, global_text_stage)

        return envs, batch, global_token_list, global_text_stage
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        global_text_stage = [0] * self.envs.num_envs
        observations, global_token_list = extract_instruction_tokens(observations, self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID, # INSTRUCTION_SENSOR_UUID = instruction
                                                  global_text_stage, max_length=instr_max_len, pad_id=instr_pad_id)
        ...
        if mode == 'eval':
            ...
            self.envs, batch, global_token_list, global_text_stage = self._pause_envs(self.envs, batch, env_to_pause, global_token_list, global_text_stage)
        ...
        last_stepk_is_subtask_stop = [False] * self.envs.num_envs
        subtask_stop_vp = [None] * self.envs.num_envs
        # print("episode subtask num: ", len(global_token_list[0]))
        for stepk in range(self.max_len): # 15
            # print("all_txt_ids ", all_txt_ids)
            # print("prev_vp ", prev_vp[0])
            # print("num_envs: ", self.envs.num_envs)
            # print("len(global_token_list[0]) ", len(global_token_list[0]))
            # print("global_text_stage[0]", global_text_stage[0])
            # print("HERE\n", (self.envs.current_episodes())[0].episode_id, global_text_stage)
            ...
            for i in range(self.envs.num_envs):
                cur_vp_i, cand_vp_i, cand_pos_i = self.gmaps[i].identify_node(
                    cur_pos[i], cur_ori[i], wp_outputs['cand_angles'][i], wp_outputs['cand_distances'][i], last_stepk_is_subtask_stop[i]
                )
                if last_stepk_is_subtask_stop[i]:
                    cur_vp_i = subtask_stop_vp[i]
                    # print("next_cur_vp_i: ", cur_vp_i)
                    # print("next_cur_pos: ", cur_pos)
            ...
            for i in range(self.envs.num_envs):
                ...
                # print("update start")
                # print("cur_vp[i], prev_vp[i]: ", cur_vp[i], prev_vp[i])
                # print("current gmap nodes: ", self.gmaps[i].node_pos.keys(), self.gmaps[i].graph_nx.nodes())
                if not last_stepk_is_subtask_stop[i]:
                    self.gmaps[i].update_graph(prev_vp[i], stepk+1,
                                            cur_vp[i], cur_pos[i], cur_embeds,
                                            cand_vp[i], cand_pos[i], cand_embeds,
                                            cand_real_pos[i])
                else:
                    self.gmaps[i].node_stepId[cur_vp[i]] = stepk+1
                # self.gmaps[i].update_graph(prev_vp[i], stepk+1,
                #                             cur_vp[i], cur_pos[i], cur_embeds,
                #                             cand_vp[i], cand_pos[i], cand_embeds,
                #                             cand_real_pos[i])
            ...
            for i, gmap in enumerate(self.gmaps):
                if cpu_a_t[i] == 0 or stepk == self.max_len - 1 or no_vp_left[i]: # 如果的当前动作是stop或步数达到上限或没有动作空间
                    ...
                    if global_text_stage[i] < len(global_token_list[i])-1 and stepk != self.max_len - 1 and not no_vp_left[i]:
                        # if i == 0: print("stop subtask")
                        last_stepk_is_subtask_stop[i] = True
                        end_episode = False
                        global_text_stage[i] = global_text_stage[i] + 1
                        any_stage_changed = True
                    else:
                        # if i == 0: print("stop episode")
                        last_stepk_is_subtask_stop[i] = False
                        end_episode = True
                    ...
                    if last_stepk_is_subtask_stop[i]:
                        # print("stop_vp: ", stop_vp, gmap.node_pos[stop_vp])
                        # print("current_vp: ", cur_vp[i], gmap.node_pos[cur_vp[i]])
                        subtask_stop_vp[i] = stop_vp

                    # ******************clear stop scores after each stage******************
                    gmap.node_stop_scores.clear()
                    # self.gmaps[i] = GraphMap(have_real_pos, # []
                    #            self.config.IL.loc_noise, # 0.5
                    #            self.config.MODEL.merge_ghost, # True
                    #            ghost_aug)
                    # prev_vp[i] = None
            ...
            if sum(dones) > 0:
                for i in reversed(list(range(self.envs.num_envs))):
                    if dones[i]:
                        ...
                        observations.pop(i)
                        global_text_stage.pop(i)
                        # graph stop
                        self.gmaps.pop(i)
                        prev_vp.pop(i)
                        last_stepk_is_subtask_stop.pop(i)
                        subtask_stop_vp.pop(i)
            ...
            # obs for next step
            observations, global_token_list = extract_instruction_tokens(observations,self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID,global_text_stage, \
                                                                         max_length=instr_max_len, pad_id=instr_pad_id)
            batch = batch_obs(observations, self.device)
            batch = apply_obs_transforms_batch(batch, self.obs_transforms)
            if any_stage_changed:
                all_txt_ids = batch['instruction'] # torch.tensor类型，环境数*80的shape，数值为整数
                all_txt_masks = (all_txt_ids != instr_pad_id)
                all_txt_embeds = self.policy.net(
                    mode='language',
