...
@baseline_registry.register_env(name="MyVLNCEDaggerEnv30")
class VLNCEDaggerEnv(habitat.RLEnv):
    ...
    def current_subtask_stage_update(self, last_subtask_stage, dis_thre=1.5):
        init_state = self._env.sim.get_agent_state()
        subtask_num = len(self._env.current_episode.goals)
        updated_subtask_stage = last_subtask_stage
        for i in range(subtask_num, last_subtask_stage-1, -1): # 这边的序号不是从0开始的，是从1开始，1表示初始时刻的stage，subtask_num表示最后一个非stop的stage
            dist = self._env.sim.geodesic_distance(
                init_state.position, self._env.current_episode.goals[i-1].position,
            )
            if dist < dis_thre:
                if i == subtask_num:
                    updated_subtask_stage = 0
                else:
                    updated_subtask_stage = i + 1
                break
        return updated_subtask_stage
    ...
    def ghost_dist_to_ref(self, ghost_vp_pos, ref_path):
        ...
        if circle_bool.sum() == 0: # no gt point within 3.0m
            ...
            cand_idxes = np.where(circle_bool * (np.arange(0,len(ref_path))>=progress))[0] # 所有在3m范围内并且是前进方向的gt路径点
            if len(cand_idxes) == 0:
                sub_goal_pos = ref_path[progress] #prev_sub_goal_pos[perm_index]
            else:
                compare = np.array(list(range(cand_idxes[0],cand_idxes[0]+len(cand_idxes)))) == cand_idxes # 检查 cand_idxes（候选点的索引）是否是连续的（例如真值轨迹中间有一个点在3m开外，前后点都在3m以内）。
                if np.all(compare): # 如果都是连续，那么选最靠后的那一个
                    sub_goal_idx = cand_idxes[-1]
                else: # 否则选取断裂之前最靠后的那一个
    ...
    def ghost_dist_to_ref_new(self, ghost_vp_pos, ref_path):
        episode_id = self._env.current_episode.episode_id
        if episode_id != self.prev_episode_id:
            self.progress = 0
            self.prev_sub_goal_pos = [0.0, 0.0, 0.0]
    ...
    def reset_current_episode(self):
        ...
        # print("info ", self._env._episode_iterator.max_scene_repetition_episodes, self._env._episode_iterator.max_scene_repetition_steps)
        return observations
    ...
    def step(self, action, vis_info, *args, **kwargs):
        ...
        if act == 4: # high to low
            ...
            # 2. stop
            observations = self._env.step(act) # 当执行env.step(stop)后，self.get_done返回True；否则是False
            if self.video_option:
                info = self.get_info(observations)
                self.video_frames.append(
                    navigator_video_frame(
