...
# -----------------------------------------------------------------------------
# GRPO_ORM LEARNING CONFIG
# -----------------------------------------------------------------------------
_C.GRPO_ORM = CN()
_C.GRPO_ORM.lr = 1e-5
_C.GRPO_ORM.iters = 5000
_C.GRPO_ORM.batch_size = 1
_C.GRPO_ORM.load_from_ckpt = True
_C.GRPO_ORM.is_requeue = False
_C.GRPO_ORM.grpo_beta = 0.01
_C.GRPO_ORM.grpo_epsilon = 0.2
_C.GRPO_ORM.max_grad_norm = 1.0
_C.GRPO_ORM.train_all = False
# -----------------------------------------------------------------------------
...
_C.IL.ACCUMULATION_STEPS = 1
_C.IL.value_loss_coef = 0.0
...
def get_config(
    ...
    """
    config = CN()
    # merge时后来者优先，对于同一个参数，值被修改为后来的config中的值，在下面的habitat_baselines.config.default._C中获取到了TASK_CONFIG相关的参数
    ...
    if config_paths:
        ...
        for config_path in config_paths:
            # merge时后来者优先，对于同一个参数，值被修改为后来的config中的值
