...
from .dino_encoder import DINOv2Encoder # 假设你放到了这里
...
class ETP(Net):
    def __init__(
        ...
        self.num_views = 4 # 定义常量
        self.pano_img_idxes = np.arange(0, self.num_views, dtype=np.int64)

        pano_angle_rad_c = (1 - self.pano_img_idxes / self.num_views) * 2 * math.pi 
    ...
    def forward(self, mode=None, 
        ...
        if mode == 'language':
            ...
            if in_train:
                ...
                batch_way_heats_regional = batch_way_heats_regional.reshape(batch_size, NUM_IMGS, 30, 12)
                ...
                for j in range(batch_size):
                    ...
                    img_idxes = ((angle_idxes.cpu().numpy()+15) // 30)
                    img_idxes[img_idxes==NUM_IMGS] = 0
                    ...
                    for k, way_act in enumerate(way_heats_act):
                        if img_idxes[k] != 0:
                            angle_pointer = (img_idxes[k] - 1) * 30 + 15
                        else:
                            angle_pointer = 0
            ...
            for j in range(batch_size):
                ...
                # for img idxes
                img_idxes = NUM_IMGS - (angle_idxes.cpu().numpy()+15) // 30        # 逆时针
                img_idxes[img_idxes==NUM_IMGS] = 0
