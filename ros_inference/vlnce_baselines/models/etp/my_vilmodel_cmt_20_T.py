...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, subtask_type_ids, token_type_ids=None, position_ids=None):
        ...
        subtask_type_embeddings = self.subtask_type_embeddings(subtask_type_ids)

        embeddings = words_embeddings + position_embeddings + token_type_embeddings + subtask_type_embeddings
...
class NextActionPrediction(nn.Module):
    def __init__(self, hidden_size, dropout_rate):
        ...
        self.net = nn.Sequential(nn.Linear(hidden_size*2, hidden_size*2),
                                 nn.ReLU(),
                                 BertLayerNorm(hidden_size*2, eps=1e-12),
...
class GlocalTextPathNavCMT(BertPreTrainedModel): # CMT可能指的是cross model transformer；Text、Path、Nav分别指代对应不同任务的输出，即该class能同时输出多种任务
    def __init__(self, config):
        ...
        self.graph_attentioned_txt_embeds_transform = ResidualTransformBlock(self.config.hidden_size, self.config.hidden_dropout_prob)
        self.global_sap_head = NextActionPrediction(self.config.hidden_size, self.config.pred_head_dropout_prob)

        if self.config.need_gsp:
            ...
            self.gsp_attentioned_txt_embeds_transform = ResidualTransformBlock(self.config.hidden_size, self.config.hidden_dropout_prob)
            self.global_stage_head = nn.Linear(config.hidden_size, config.subtask_num+1)

        self.subtask_num = config.subtask_num

        self.need_T = config.need_T
        if self.need_T:
            print("Init GlocalTextPathNavCMT with T = ", config.initial_T)
            initial_log_temp = np.log(config.initial_T)
            self.log_temp = nn.Parameter(torch.tensor(initial_log_temp, dtype=torch.float32))
        else:
            print("Init GlocalTextPathNavCMT without T")
        ...
        if config.fix_lang_embedding:
            ...
            print("LANG EMBEDDING can be trained!!!!!!!!!!!!!!!!!!!!!!!!")
        if config.fix_pano_embedding:
            print("FIX PANO EMBEDDING!!!!!!!!!!!!!!!!!!!!!!!!")
            for k, v in self.img_embeddings.named_parameters():
                v.requires_grad = False
        else:
            print("PANO EMBEDDING can be trained!!!!!!!!!!!!!!!!!!!!!!!!")
    
    def forward_txt(self, txt_ids, subtask_type_ids, txt_masks):
        txt_token_type_ids = torch.zeros_like(txt_ids)
        txt_embeds = self.embeddings(txt_ids, subtask_type_ids, token_type_ids=txt_token_type_ids)
    ...
    def forward_navigation(
        ...
        gmap_masks, gmap_visited_masks, gmap_pair_dists, subtask_lens
    ):
        # global branch
        gmap_embeds = gmap_img_fts + \
        ...
        if self.need_T:
            ...
            global_logits = global_logits / (current_temp + 1e-8)
        ...
        if self.config.need_gsp:
            ...
            global_stage_logits.masked_fill_(gen_seq_masks(subtask_lens, max_len=self.subtask_num+1).logical_not(), -float('inf'))
        else:
            global_stage_logits = None

        if self.need_T:
            outs = {
            'gmap_embeds': gmap_embeds, # 在外部并没有使用到这个返回值
            'global_logits': global_logits,
            'global_stage_logits': global_stage_logits,
            'T': current_temp.item()
            }
        else:
            outs = {
    ...
    def forward(self, mode, batch, **kwargs):
        print("forward fuction in GlocalTextPathNavCMT")
        if mode == 'language':
