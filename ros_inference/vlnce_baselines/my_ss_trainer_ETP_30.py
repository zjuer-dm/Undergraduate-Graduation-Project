...
from .utils import get_camera_orientations4
from .utils import (
...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _set_config(self):
        ...
        camera_orientations = get_camera_orientations4()
    ...
    def _initialize_policy(
        ...
        # TODO: 根据任务类型选择不同的预训练权重
        cwp_fn = 'data/wp_pred/check_cwp_bestdist_hfov63' if self.config.MODEL.task_type == 'rxr' else 'data/wp_pred/check_val_best_seek_dino'
    ...
    def _vp_feature_variable(self, obs):
        ...
        for i in range(self.envs.num_envs):
            ...
            cand_idxes = np.zeros(num_views , dtype=np.bool)
            ...
            nav_types += [0] * (num_views -np.sum(cand_idxes))
    ...
    def _train_interval(self, interval, ml_weight, sample_ratio):
        ...
        # --- NEW: 从配置中读取累积步数 ---
        # 目标: ACCUMULATION_STEPS = 2
        accumulation_steps = self.config.IL.ACCUMULATION_STEPS 
        if accumulation_steps < 1:
            accumulation_steps = 1
        # --- END NEW ---

        for idx in pbar:
            # 只有在累积周期开始时（或 idx=0 时），才清零梯度
            ...
            # 1. 计算梯度，并按累积步数进行缩放
            # scaled_loss = self.loss / G
            scaled_loss = self.loss / accumulation_steps 
            
            # 使用 scale.backward() 计算梯度
        ...
        # 处理未完成的累积周期（如果 interval 不是 ACCUMULATION_STEPS 的整数倍）
        if interval % accumulation_steps != 0:
            logger.warning(f"Interval {interval} is not divisible by {accumulation_steps}. Performing final update with partial batch.")
            self.scaler.step(self.optimizer) 
            self.scheduler.step()
            self.scaler.update()
