...
def get_pixel_location(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        print(f'disparity_image:', disparity_image[y][x])
        point3d = model.projectPixelTo3d((x,y), disparity_image[y][x])
        print("3d_point:", point3d)
        print("distance:", numpy.linalg.norm(numpy.array([point3d[0],point3d[1],point3d[2]]), ord=2), " m")
        print("Z:", model.getZ(disparity_image[y][x]), " m")

def listener():
    global disparity_image, model
    rospy.init_node('image_subscriber', anonymous=True)

    cv2.namedWindow("Image")
    cv2.setMouseCallback('Image', get_pixel_location)

    left_msg = rospy.wait_for_message(f"/{camera_prefix}/left/camera_info", sensor_msgs.msg.CameraInfo, timeout=5)
    right_msg = rospy.wait_for_message(f"/{camera_prefix}/right/camera_info", sensor_msgs.msg.CameraInfo, timeout=5)
    
    print("baseline = ", right_msg.P[3])
    model.fromCameraInfo(left_msg, right_msg)

    while not rospy.is_shutdown():
        data = rospy.wait_for_message(f"/{camera_prefix}/left/image_raw", Image, timeout=5)
        data2 = rospy.wait_for_message(f"/{camera_prefix}/disparity", DisparityImage, timeout=5)

        bridge = CvBridge()
        cv_image = bridge.imgmsg_to_cv2(data, "mono8")
        cv2.imshow("Image", cv_image)

        disparity_image = bridge.imgmsg_to_cv2(data2.image, "32FC1")
        cv2.imshow("Disparity", disparity_image / 128.0)  # 缩放视差图像以适应显示
